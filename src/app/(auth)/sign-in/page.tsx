  ...
  async function connectWallet() {
    ...
    try {
      ...
      if (typeof window !== "undefined" && (window as unknown as Record<string, unknown>).freighter) {
        // Freighter is installed — in production call freighterApi.getPublicKey()
        // For demo, simulate a successful connection after a brief delay
        await new Promise((r) => setTimeout(r, 1200));
        const demoAddress = "GDEMO...XLM123";
        ...
        setError("Freighter wallet not found. Please install the Freighter browser extension to continue.");
      }
    } catch {
      setWalletState("error");
  ...
  return (
    <div className="w-full max-w-md space-y-8">
      ...
      <div className={`border-struct bg-[#111113] p-8 space-y-6 transition-opacity ${step === 1 ? "opacity-40 pointer-events-none select-none" : ""}`}>
        ...
        {role && (
          <Link href="/dashboard" className="block w-full btn-primary text-center py-4">
            Enter as {role === "client" ? "Client" : "Freelancer"} →
          </Link>
        )}
      </div>
