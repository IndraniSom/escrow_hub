...
export default function DashboardPage() {
  ...
  const [stats, setStats] = useState({ escrowTotal: "0", activeProjects: 0, completedMilestones: 0 });
  const [transactions, setTransactions] = useState<{ type: string; projectName: string; txHash: string; amount: string }[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setTransactions(
          (txRes.data || []).slice(0, 5).map((tx: Transaction) => ({
            type: tx.type,
    ...
    <div className="p-8 md:p-12 max-w-6xl mx-auto space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 border-struct">
        <div className="p-8 border-struct-b md:border-struct-b-0 md:border-struct-r bg-[#09090b]">
          ...
          <p className="text-4xl font-display font-black text-white">${stats.escrowTotal} <span className="text-lg text-[#a1a1aa] font-mono tracking-widest">USDC</span></p>
        </div>
        <div className="p-8 border-struct-b md:border-struct-b-0 md:border-struct-r bg-[#09090b]">
          <h3 className="text-orange-600 text-xs font-bold uppercase tracking-widest mb-4">Active Projects</h3>
          <p className="text-4xl font-display font-black text-white">{String(stats.activeProjects).padStart(2, "0")}</p>
        </div>
        <div className="p-8 bg-[#09090b]">
          ...
          <p className="text-4xl font-display font-black text-white">{String(stats.completedMilestones).padStart(2, "0")}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="border-struct bg-[#09090b]">
          ...
          {transactions.map((tx, i) => (
            <div key={i} className={`grid grid-cols-12 gap-4 p-4 ${i < transactions.length - 1 ? "border-struct-b" : ""} items-center hover:bg-[#111113] transition-colors`}>
