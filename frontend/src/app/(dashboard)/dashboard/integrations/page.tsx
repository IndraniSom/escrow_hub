...
const INTEGRATIONS_CONFIG: { id: string; name: string; description: string; icon: React.ElementType; scopes: string[] }[] = [
  { id: "slack", name: "Slack", description: "Receive real-time notifications when milestones are approved, payments are released, or disputes are raised.", icon: FaSlack, scopes: ["channels:read", "chat:write"] },
  { id: "email", name: "Email (SMTP/API)", description: "Get email notifications for contract events and milestone status updates directly in your inbox.", icon: Mail, scopes: ["email.send", "email.read"] },
  { id: "notion", name: "Notion", description: "Automatically sync project milestones, escrow statuses, and timeline events to your Notion databases.", icon: SiNotion, scopes: ["databases:read", "pages:write"] },
];
...
function StatusBadge({ status }: { status: IntegrationStatus }) {
  ...
  return <span className={`px-2 py-1 text-[10px] uppercase tracking-widest font-bold border ${map[status]}`}>{label[status]}</span>;
}
...
export default function IntegrationsPage() {
  ...
  async function handleConnect(id: string) {
    ...
    try {
      await api.integrations.connect({ platform: id, code: "oauth_callback_code" });
      setIntegrations((prev) =>
        prev.map((i) => (i.id === id ? { ...i, status: "connected", detail: `Connected to ${i.name}` } : i))
      );
    } catch {
  ...
  async function handleDisconnect(id: string) {
    try {
      ...
      setIntegrations((prev) => prev.map((i) => (i.id === id ? { ...i, status: "disconnected", detail: null } : i)));
    } catch {}
  }
